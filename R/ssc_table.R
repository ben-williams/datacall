...
#' @export ssc_table
#'
#' @examples ssc_table(year, model)
ssc_table <- function(year, model){

  option(scipen = 999)
  fsc = read.csv(here::here(year, "data", "output", "survey_size_comp.csv"))
  ...
  dplyr::bind_rows(comp, samps) %>%
    write.csv(here::here(year, model, "tables", "tbl_10_10.csv"), row.names = FALSE)
