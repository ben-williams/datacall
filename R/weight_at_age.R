...
#'
#' @examples
weight_at_age <- function(year, admb_home, rec_age, plus_age){
  ...
  for(a in 1:nages){
    ...
    SD_Wbar[a]<-sqrt(sum(r_la[a,]^2 * V_Wbar_la[a,] +
                       ...
                       theta_a[a]^2) * sqrt(length(subset(age_data_1$weight,
  ...
  WaA_stats<-WaA_stats[-r,]
  # Write data
  write.csv(WaA_stats, here::here(year, "data/output/waa_stats.csv"), row.names = FALSE)
  ...
  # Write data
  write.csv(lw_mdl_data, here::here(year, "data/output/wal_stats.csv"), row.names = FALSE)
  ...
  Wbar = Winf*(1-exp(-k*(rec_age:plus_age - t0)))^beta_lw
  Wbar[max(length(rec_age:plus_age))] = 0.5*(Wbar[max(length(rec_age:plus_age))]+Winf)
